...
Overview
  ...
  to limitations listed below) as per RFC 2407, 2408, 2409.

  The library implements ISAKMP packet processing code, IKE state
  management code and various miscellaneous functionality such as
  handling of packet retransmissions, SA lifetime tracking, etc.
...
Features
  ...
  * ESP IPsec SA (no IPCOMP or AH)
  ...
  a packet retransmissions and SA lifetime tracking.

  The code concerned with the cryptography is decoupled from the library 
  and is accessed via a generic interface. The implementation defaults 
  to the use of OpenSSL library, yet a custom and/or hardware 
  implementations are as easily accommodated.
...
How to build
  ...
  Linux
   ...
   to build - issue the following in root folder of the package:
...
History
  ...
  * Refactored inbound processing code - added generalized payloads'
  ...
  0.9.4.2
  * Resolved number of portability issues including the use of
...
Docs
  ...
  Refer to contents of /x4/ike/docs directory for usage and design guides.
