 ...
 *
 *    $Id: test.c,v 1.9 2003/04/04 19:56:54 alex Exp $
 */
 ...
 *  
 *  Once ready, run the test and you verify it completes both phases of 
 *  the negotiation. Also try running it in NAT-T mode, by commenting out
...
{ 
...

  {
    ...
    sel->l.ip.lo.x =
    sel->l.ip.hi.x = inet_addr("10.0.0.1");
...
x4s_buf on_ph1_get_pubkey(void * ctx)
{ 
  x4s_buf r = { 0 };
